...
const metronome = {
  ...
  ensureNodes() {
    synth.ensureContext();
    if (!this.gainNode) {
      // Own gain, straight to destination: clicks bypass the gesture-controlled
      // filter/volume chain so the beat stays audible when no chord is playing.
      this.gainNode = synth.ctx.createGain();
      this.gainNode.gain.value = 0.4;
      this.gainNode.connect(synth.ctx.destination);
    }
  },

  scheduleClick(beat, time) {
    const osc = synth.ctx.createOscillator();
    const env = synth.ctx.createGain();
    ...
    const delayMs = Math.max(0, (time - synth.ctx.currentTime) * 1000);
    setTimeout(() => {
  ...
  scheduler() {
    while (this.nextNoteTime < synth.ctx.currentTime + this.scheduleAheadSec) {
  ...
  start() {
    ...
    this.isRunning = true;
    this.currentBeat = 0;
    this.nextNoteTime = synth.ctx.currentTime + 0.05;
